...
class RoverState():
    def __init__(self):
      ...
      # The stop_forward and go_forward fields below represent total count
        ...
        # Rover motion attributes
        self.mode = 'forward'  # Current mode (can be forward or stop)
        self.vel                = None  # Current velocity
        self.steer            = 0  # Current steering angle
        self.throttle         = 0  # Current throttle value
        self.brake           = 0  # Current brake value
        self.throttle_set  = 1.5  # Throttle setting when accelerating
        self.brake_set     = 10  # Brake setting when braking
        self.max_vel       = 3  # Maximum velocity (meters/second)
        self.steer_list      = []
        self.grid_x           = 0         #grid location rover is in
        self.grid_y           = 0         #grid location rover is in
        self.grid_list        = []         #list of grid locations
        self.weight          = 1
        self.weight_map  = np.zeros((20,20), dtype=int)

        # Image attributes
        self.vision_image = np.zeros((160, 320, 3), dtype=np.float)
        self.worldmap      = np.zeros((200, 200, 3), dtype=np.float)
        # self.gridmap         = np.zeros((200, 200, 3), dtype=np.float)

        # Rock sample attributes
...
def telemetry(sid, data):
    ...
    if data:
        ...
        if args.image_folder != '':
            timestamp = datetime.utcnow().strftime('%Y_%m_%d_%H_%M_%S_%s')[:-3]
